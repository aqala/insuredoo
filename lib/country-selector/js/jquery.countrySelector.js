    ...
    init: function (options) {
      ...
      return this.each(function () {
        ...
        if (options.listMode == 1) {
          var _countries = _valueSet.length == 0 ? NATIONALITIES : $.grep(NATIONALITIES, function (country) {
            return _valueSet.indexOf(country[_codeProp]) >= 0;
          });
        } else {

          var _countries = _valueSet.length == 0 ? COUNTRIES : $.grep(COUNTRIES, function (country) {
            return _valueSet.indexOf(country[_codeProp]) >= 0;
          });
        }
        _countries = options.listMode == 1 ? NATIONALITIES : COUNTRIES
